...
async function manageOrders(config) {
    ...
    for (const terminal of terminals) {
        ...
        // Add mineral orders
        for (const mineral of mineralOrders) {
            if (Object.keys(market.buyPrices).includes(mineral) && !orders.buy[mineral] && market.buyPrices[mineral] * 1000 > 0) {
                ...
                if (market.marketType === 'fixed') {
                    buy.price = parseInt(buyPrice.toFixed(0))
                } else {
                    buy.price = parseInt((buyPrice * (Math.random() * (+1.2 - +0.8) + +0.8)).toFixed(0))
                }
                delete buy._id
                // Add to queue
                queued.push(buy)
                ...
                // Remove bad orders
                await db['market.orders'].removeWhere({_id: orders.buy[mineral]._id})
            }
            if (Object.keys(market.sellPrices).includes(mineral) && !orders.sell[mineral] && market.sellPrices[mineral] * 1000 > 0) {
                ...
                // Handle pricing
                if (market.marketType === 'fixed') {
                    sell.price = parseInt(sellPrice.toFixed(0))
                } else {
                    sell.price = parseInt((sellPrice * (Math.random() * (+1.2 - +0.8) + +0.8)).toFixed(0))
                }
                delete sell._id
                // Add to queue
                queued.push(sell)
                ...
                // Remove bad orders
                await db['market.orders'].removeWhere({_id: orders.sell[mineral]._id})
            }
        }
        ...
        if (market.randomBoostSales) {
            if (!boosts[0].concat(boosts[1]).concat(boosts[2]).find((b) => orders.sell[b] && !Object.keys(market.sellPrices).includes(b))) {
                ...
                if (orders.sell[onSale].created + 1000 < gameTime) {
                    await db['market.orders'].removeWhere({_id: orders.sell[onSale]._id})
                }
            }
        }
        // Input new orders
		for (const order of queued) {
			await db['market.orders'].update({ type: order.type, roomName: order.roomName, resourceType: order.resourceType }, { $merge: order }, { upsert: true })
		}
    }
}
...
async function updateOrders(config) {
	...
	for (const terminal of NPCTerminals) {
                    ...
                    if (market.marketType !== 'dynamic') {
                        if (orders.sell[mineral]) {
                            ...
                            if (market.marketType === 'random') {
                                ...
                                order.price = parseInt((baseSellPrice * (Math.random() * (+1.2 - +0.8) + +0.8)).toFixed(0))
                            } else {
                                order.price = parseInt(baseSellPrice.toFixed(0))
                            }
                            queued.push(order)
                        }
                        if (orders.buy[mineral]) {
                            ...
                            if (market.marketType === 'random') {
                                ...
                                order.totalAmount = randomAmount
                                order.amount = randomAmount
                                order.price = parseInt((baseBuyPrice * (Math.random() * (+1.2 - +0.8) + +0.8)).toFixed(0))
                            } else {
                                order.price = parseInt(baseBuyPrice.toFixed(0))
                            }
                            queued.push(order)
                        }
                    } else {
                        ...
                        if (orders.buy[mineral]) {
                            ...
                            order.amount = market.buyAmount * (playerTerminals.length + 1)
							//order.active = true
                            order.price = parseInt(((market.buyPrices[mineral] * 1000) * saturationModifier).toFixed(0))
                            // Check if a sell order exists, and make sure the buy order does not exceed it
                            if (lowestOrder[mineral] && order.price > lowestOrder[mineral]) order.price = parseInt((lowestOrder[mineral] * 0.98).toFixed(0));
                        ...
                        if (orders.sell[mineral]) {
                            ...
                            order.price = parseInt((baseSellPrice * saturationModifier).toFixed(0))
                            queued.push(order)
                        }
                    }
                }
        ...
        // Update orders
		for (const { _id, ...order } of queued) {
			await db['market.orders'].update({ _id }, { $set: order })
		}
    }
}
